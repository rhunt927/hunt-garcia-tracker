...
- [x] **Banistmo (Panama) PDF import (2026-09-01)** — checking-account statements, detected via "Banistmo" in the PDF text; single combined table (no credit/debit sections) with day-first dates ("31 Aug 2026") and a leading "-" on the amount marking a withdrawal; detail text that overflows wraps onto the line *after* the amount/balance instead of before it, unlike every other supported bank — new "Banistmo Checking" payment method auto-registers on import; verified against a real statement
